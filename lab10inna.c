...
for(int z=0;z<5;z++){
	...
	for( int blocksize=4;blocksize<=128;  blocksize*=2){
			...
			printf("Blocked: MX_SIZE:%d \t BLOCK_SIZE:%d time = %2.3lf GFLOPS = %.3lf\n", n, blocksize, time_tabl[1],(double)ff/time_tabl[1]/1.0e9 );
					...
					// for (i=0;i<n*n;++i) 
					// 	if (fabs(f[i]-g[i])>1.0e-9) {printf("Error blocked!\n"); goto rtrn;}
...
for(register int i=0;i<n;i+=4)
	...
	for(register int k=0;k<n;++k){
	    cv=_mm256_add_pd(cv, _mm256_mul_pd(_mm256_loadu_pd(A+i+k*n),_mm256_loadu_pd(B+k+j*n))); 	
...
for(int s=0;s<5;s++)printf("gflopsy = %.3lf\n", suma_u[s]/5.0e9 );
